...
class _QueueDisplayScreenState extends State<QueueDisplayScreen> {
      ...
      child: Focus(
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            final needsRotation = constraints.maxWidth > constraints.maxHeight;
            final portraitWidth =
                needsRotation ? constraints.maxHeight : constraints.maxWidth;
            final portraitHeight =
                needsRotation ? constraints.maxWidth : constraints.maxHeight;

            Widget content = ScreenUtilInit(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    _buildNowServing(current),
                                    SizedBox(height: 22.h),
                                    _buildNextUp(next),
                                    SizedBox(height: 22.h),
                                    _buildWaitingList(waiting),
            ...
            // شاشة العرض مثبتة عمودياً: إذا Windows بلّغ أفقياً ندوّر المحتوى
            if (needsRotation) {
              content = RotatedBox(
                quarterTurns: 3,
                child: content,
  ...
  Widget _buildNowServing(QueueEntry? current) {
    // أبعاد Figma للكونتينر الأزرق: 707 × 551
    return Center(
      child: Container(
        width: 707.w,
        height: 551.h,
  ...
  Widget _buildNowContent(QueueEntry current) {
    return Padding(
      padding: EdgeInsets.fromLTRB(40.w, 32.h, 40.w, 28.h),
      child: Column(
        children: [
          _sectionTitle(
          'الآن',
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    _MotionLines(mirror: true, size: 84.w),
                    SizedBox(width: 16.w),
                    Flexible(
                      child: FittedBox(
                    ...
                    SizedBox(width: 16.w),
                    _MotionLines(mirror: false, size: 84.w),
                  ],
                ),
                SizedBox(height: 8.h),
                _NameDivider(),
  ...
  Widget _buildNextUp(QueueEntry? next) {
    // أبعاد Figma: 953 × 280، radius 40
    // Shadow: X0 Y0 blur 25.4 spread -2، أسود 25%
    return Center(
      child: Container(
        width: 953.w,
        height: 280.h,
                ...
                children: [
                  Align(
                    ...
                    child: Container(
                      ...
                      height: 180.w,
                      decoration: BoxDecoration(
                        color: const Color(0xFFCEDEEA).withValues(alpha: 0.45),
  ...
  Widget _waitingColumn(List<QueueEntry> entries) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (count == 1) {
          itemH = math.min(itemH, available);
        } else {
          final leftover = available - itemH * count;
          if (leftover >= 0) {
            gap = math.min(12.h, leftover / gaps);
          } else {
            // يتقلص ارتفاع الصفوف ليتسع داخل الكونتينر بدون overflow
            itemH = math.max(30.h, (available - gaps * 2.h) / count);
            final afterItems = available - itemH * count;
            gap = afterItems > 0 ? afterItems / gaps : 0.0;
          }
        }
