...
async def initialize_implant_stages(patient_id: str, registration_date: datetime, doctor_id: str) -> List[ImplantStage]:
    """تهيئة مراحل زراعة الأسنان للمريض - إنشاء المرحلة الأولى فقط."""
    patient = await Patient.get(OID(patient_id))
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
    
    # التحقق من وجود مراحل سابقة
    existing_stages = await ImplantStage.find(ImplantStage.patient_id == patient.id).to_list()
    if existing_stages:
        # إذا كانت المراحل موجودة بالفعل، لا نقوم بإعادة تهيئتها
        return existing_stages
    
    # إنشاء المرحلة الأولى فقط (مرحلة زراعة الاسنان)
    ...
    # إنشاء ImplantStage للمرحلة الأولى فقط
    stage = ImplantStage(
        patient_id=patient.id,
        stage_name=first_stage_name,
        scheduled_at=base_date,
...
async def get_implant_stages(patient_id: str) -> List[ImplantStage]:
    """جلب جميع مراحل زراعة الأسنان للمريض."""
    patient = await Patient.get(OID(patient_id))
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
    
    stages = await ImplantStage.find(
        ImplantStage.patient_id == patient.id
    ).sort("+scheduled_at").to_list()
    
    # إذا لم تكن هناك مراحل، نرجع قائمة فارغة
    return stages
...
async def _get_next_stage_date(current_stage_name: str, current_stage_date: datetime, doctor_id: str) -> datetime:
...
async def complete_stage(patient_id: str, stage_name: str, doctor_id: str) -> ImplantStage:
    ...
    # التحقق من التسلسل: يجب أن تكون المرحلة السابقة مكتملة
    try:
    ...
    # إنشاء المرحلة التالية تلقائياً إذا لم تكن موجودة
    try:
        current_index = IMPLANT_STAGES.index(stage_name)
        if current_index < len(IMPLANT_STAGES) - 1:
            # هناك مرحلة تالية
            ...
            # التحقق من وجود المرحلة التالية
            ...
            if not existing_next_stage:
                # حساب تاريخ المرحلة التالية
                next_date = await _get_next_stage_date(stage_name, stage.scheduled_at, doctor_id)
                
                # إنشاء Appointment للمرحلة التالية
                ...
                # إنشاء ImplantStage للمرحلة التالية
...
async def uncomplete_stage(patient_id: str, stage_name: str, doctor_id: str) -> ImplantStage:
